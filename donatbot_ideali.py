...
CARD_NUMBER = "9860 3566 3889 1529"  # <-- to'lov uchun ko'rsatiladigan karta
# ==================================================
...
def check_spam(chat_id, text=None):
    ...
    Agar bekor qilish yoki /start bo'lsa — ularni istisno qiladi (True qaytaradi).
    ...
    if is_blocked(chat_id):
        ...
        try:
            bot.send_message(chat_id, "🚫 Siz bloklangan ekansiz. Iltimos admin bilan bog‘laning.")
        except Exception:
            pass
        return False
        ...
        if remaining[chat_id] <= 0:
            ...
            try:
                bot.send_message(chat_id,
                    f"⚠️ Iltimos, botga tez-tez so‘rov yubormang!\nQolgan urinishlar: {remaining[chat_id]}")
            except Exception:
                pass
            return False
...
def handle_text(message):
    ...
    if ud:
        ...
        if step == "await_ml_amount" and text in ML_AMOUNTS:
            ud["amount"] = text
            ud["step"] = "await_id"
            bot.send_message(chat_id, "🆔 Iltimos, MLBB Game ID ni kiriting (raqam):", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add(BTN_BACK))
            return
...
if __name__ == "__main__":
    ...
    # infinity_polling yoki polling ishlatish mumkin
